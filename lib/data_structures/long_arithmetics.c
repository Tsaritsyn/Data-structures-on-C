...
static LongNum* div_long_absolutes(const LongNum* nom, const LongNum* denom) {
...
//    extracting the last digits of the nominator into a sliding window
    LongNum* cur_nom = new_empty_long_num(denom_len);
    cur_nom->sign = PLUS;
    for (i = 0; i < denom_len - 1; i++)
        append_to_array(cur_nom->digits, nom_reverted->digits->elements[i]);
    revert_array(cur_nom->digits);

    LongNum* result = new_empty_long_num(len_result);
    result->sign = PLUS;

    for (i = denom_len - 1; i < nom_len; i++) {
//        append the current nominator digit to the end of the sliding window
        long_shift_left(&cur_nom, 1);
        cur_nom->digits->elements[0] = nom_reverted->digits->elements[i];

        int relation = compare_long_absolutes(cur_nom, denom);
//        if the window is less than denom, set 0 to the current digit's result and continue extracting nom's digits
        if (relation < 0) append_to_array(result->digits, 0);
//        if the window is exactly equal to the denom, nullify it and set current result's digit to 1
        else if (relation == 0) {
            append_to_array(result->digits, 1);
            delete_long_num(cur_nom);
            cur_nom = new_long_zero();
        }
...
//            we determine the multiplier via the binary search
            while (1) {
                ...
                int temp_relation = compare_long_absolutes(multiplied_denom, cur_nom);
...
//            this multiplier will be the current digit of the result
            append_to_array(result->digits, mult);
            sub_from_long_num(&cur_nom, multiplied_denom);
            delete_long_num(multiplied_denom);
        }
    }
...
//    since the digits in the result come in the backwards order, we need to revert it
    revert_array(result->digits);
...
//    and the division of numbers with equal absolute values is 1 or -1
    ...
    result = div_long_absolutes(nom, denom);
...
static LongNum* mod_long_absolutes(const LongNum* nom, const LongNum* denom) {
    size_t denom_len = denom->digits->length;
    size_t nom_len = nom->digits->length;
    size_t i;
    ...
    LongNum* result = new_empty_long_num(denom_len);
    result->sign = PLUS;
    for (i = 0; i < denom_len - 1; i++)
    append_to_array(result->digits, nom_reverted->digits->elements[i]);
    revert_array(result->digits);

    for (i = denom_len - 1; i < nom_len; i++) {
        long_shift_left(&result, 1);
        result->digits->elements[0] = nom_reverted->digits->elements[i];

        int relation = compare_long_absolutes(result, denom);
        if (relation < 0) {}
        else if (relation == 0) {
            ...
            while (1) {
                ...
                int temp_relation = compare_long_absolutes(multiplied_denom, result);
                if (temp_relation == 0) break;
                else if (temp_relation < 0) {
                    if (right - left == 1) break;
                    else left = mult;
            ...
            sub_from_long_num(&result, multiplied_denom);
            delete_long_num(multiplied_denom);
        }
    }

    if (compare_long_absolutes(result, denom) == 0) {
        delete_long_num(result);
        result = new_long_zero();
    }
...
//    different signs it gives their sum
        ...
        if (nom->sign == denom->sign) return copy_long_num(nom);
        else return add_long_num(nom, denom);
    }
    else {
        LongNum* result = mod_long_absolutes(nom, denom);
